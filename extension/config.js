export const CONFIG = {
  supabaseUrl: 'https://ypegwpoykntbpburndjf.supabase.co',
  supabasePublishableKey: 'sb_publishable_sQc1cYuZ43yi8BBJJlclng_JLo8TcKW',
  licenseActivateUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/license-activate',
  licenseStatusUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/license-status',
  historyClearUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/history-clear',
  githubConnectStartUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/github-connect-start',
  githubSyncUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/github-sync',
  githubManageUrl: 'https://github.com/settings/installations',
  agentFunctionUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/agent',
  transcribeFunctionUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/transcribe-audio',
  projectIntegrationsUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/project-integrations',
  supabaseConnectStartUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/supabase-connect-start',
  supabaseAccountUrl: 'https://ypegwpoykntbpburndjf.supabase.co/functions/v1/supabase-account',
};
